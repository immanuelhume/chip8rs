...
fn main() -> Result<(), io::Error> {
    ...
    let mut emulator = Emulator::new(time::Duration::from_millis(10), terminal, rom);
...
struct Emulator {
    ...
    // These are fields related only to the UI.
    terminal: Terminal<CrosstermBackend<io::Stdout>>,
    instructions: Vec<Instruction>,
}

trait HandlerFunc {
    fn next(&mut self) -> dyn HandlerFunc;
}

impl Emulator {
    fn new(poll_timeout: time::Duration, terminal: Terminal<CrosstermBackend<io::Stdout>>, rom: Vec<u8>) -> Self {
        let mut mem = [0; 0x1000];
        mem.write(0x100, &FONT_DATA).unwrap(); // write font sprits to 0x100
        mem.write(0x200, &rom).unwrap(); // write c8 file contents to 0x200
        ...
        Self {
            ...
            emu_state: EmulatorState::Normal,
    ...
    fn run(&mut self) -> Result<(), Box<dyn error::Error>> {
        // Do the terminal UI stuff.
        terminal::enable_raw_mode()?;
        let mut stdout = io::stdout();
        crossterm::execute!(stdout, EnterAlternateScreen, EnableMouseCapture)?;

        loop {
            // In each iteration, we'll draw the UI, then first check the emulator state, then the
            // game state. It is important that this loop never blocks!!
            self.terminal.draw(|f| {
                ui(f, &self.screen.pixels, &self.instructions, (self.pc - 0x200) / 2);
            })?;
            ...
            match self.emu_state {
                EmulatorState::Paused => {
                    ...
                    match key {
                        None => (),
                        Some(key) => match key {
                            KeyCode::Char('i') => self.emu_state = EmulatorState::Debug,
                            KeyCode::Char('o') => self.emu_state = EmulatorState::Stopped,
                            KeyCode::Char('p') => self.emu_state = EmulatorState::Normal,
                            _ => (),
                        },
                    }
                }
                EmulatorState::Normal => {
                    let mut should_exec_next = true;
                    let key = try_get_key(self.poll_timeout);

                    match self.game_state {
                        GameState::WaitForKey(reg) => match key {
                            None => should_exec_next = false,
                            Some(code) => {
                    ...
                    match key {
                        ...
                        Some(key) => match key {
                            KeyCode::Char('i') => self.emu_state = EmulatorState::Debug,
                            KeyCode::Char('o') => self.emu_state = EmulatorState::Stopped,
                            KeyCode::Char('p') => self.emu_state = EmulatorState::Paused,
                            _ => (),
                        },
                    }
                }
                EmulatorState::Debug => {
                    ...
                    match key {
                        None => (),
                        Some(key) => match key {
                            KeyCode::Char('i') => self.emu_state = EmulatorState::Normal,
                            KeyCode::Char('o') => self.emu_state = EmulatorState::Stopped,
                            KeyCode::Char('p') => self.emu_state = EmulatorState::Paused,
                            KeyCode::Enter if should_exec_next => {
                                let next = self.fetch_next_and_decode().expect("could not decode next instruction");
                                self.exec(next);
                            }
                            _ => (),
        ...
        crossterm::execute!(self.terminal.backend_mut(), LeaveAlternateScreen, DisableMouseCapture)?;
...
mod test_emulator {
    use crate::Memory;
    use crate::{split_u16_into_bytes, Emulator, Instruction};
    use std::{io, time};
    ...
    fn init() -> (Emulator, Vec<u16>, Vec<u8>) {
        ...
        let backend = CrosstermBackend::new(stdout);
        let terminal = Terminal::new(backend).unwrap();
        let emu = Emulator::new(time::Duration::MAX, terminal, vec![]);
    ...
    fn fetch_and_decode() {
        ...
        emu.mem.write(0x200, &instructions_in_bytes[..]).unwrap();
...
/// Virtual representation of a screen.
struct Screen {
    pixels: [bool; 64 * 32],
}
...
impl Screen {
    ...
    fn flip(&mut self, i: usize, j: usize) -> Option<bool> {
        ...
        let old = self.pixels[i * 64 + j];
        self.pixels[i * 64 + j] = !old;
        Some(old)
    }
...
mod test_pixels {
    ...
    fn can_set_out_of_screen() {
        let mut pxs = Screen::new();
        pxs.set(33, 65);
    }

    #[test]
    fn clear() {
        let mut scrn = Screen::new();
        ...
        assert_ne!(scrn.pixels, [false; 64 * 32]);

        scrn.clear();
        assert_eq!(scrn.pixels, [false; 64 * 32]);
    }
}
